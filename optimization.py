...
def solve_optimization_for_gridcell_general_min_max_distance(es_vals, rcps, es_weights=None, m_upper_bounds=None, m_lower_bounds=None, additional_constraints=None, lambda_opt=0, infeasible_management_idxs=[]):
    ...
    if len(rows_all_zero) > 0:
        print('Warning: ' + str(len(rows_all_zero)) + ' rows where found where an indicator for an RCP had all zeros for each strategy. Removing this constraint.')
        lhs_ineq = np.delete(lhs_ineq, rows_all_zero, axis=0)
        rhs_ineq = np.delete(rhs_ineq, rows_all_zero, axis=0)
